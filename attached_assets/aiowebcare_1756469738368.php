<?php
/**
 * Plugin Name: AIOWebcare – WordPress Maintenance Plugin
 * Plugin URI:  https://aiowebcare.com/
 * Description: All-in-One WordPress maintenance solution. Handle updates, backups, security, uptime monitoring, and performance optimization from one dashboard.
 * Version:     1.0.0
 * Author:      Shivaji Khandagale
 * Author URI:  https://aiowebcare.com/
 * License:     GPLv2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: aiowebcare
...
class AIOWebcare_Manager {
    ...
    public function register_routes() {
        ...
        register_rest_route($this->api_namespace, '/optimization/revisions-count', array(
            'methods' => 'GET',
            'callback' => array($this, 'get_revisions_count_detailed'),
            'permission_callback' => array($this, 'verify_api_key')
        ));

        register_rest_route($this->api_namespace, '/optimize', array(
            'methods' => 'POST',
            'callback' => array($this, 'perform_optimization'),
            'permission_callback' => array($this, 'verify_api_key_with_admin_bypass')
        ));
    }
    ...
    public function get_optimization_overview($request) {
        ...
        // Get table count
        $table_count = $wpdb->get_var("SHOW TABLES");
        ...
        // Get trash items
        $trash_items = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_status = 'trash'");
    ...
    public function get_database_stats($request) {
        ...
        $tables_raw = $wpdb->get_results($tables_query, ARRAY_A);
        $tables = array();
        $total_size_mb = 0;
        
        // If information_schema fails, use alternative approach
        if (empty($tables_raw) || $wpdb->last_error) {
            // Get table names using SHOW TABLES
            $table_names = $wpdb->get_col("SHOW TABLES");
            
            foreach ($table_names as $table_name) {
                // Get table status for each table
            ...
            // Calculate overhead using SHOW TABLE STATUS
            $overhead_result = 0;
            foreach ($table_names as $table_name) {
            ...
            // Original information_schema approach worked
            foreach ($tables_raw as $table) {
                ...
                $total_size_mb += floatval($table['size_mb']);
            }
            
            // Calculate overhead using information_schema
            $overhead_query = "
                SELECT ROUND(SUM(data_free)/1024/1024, 2) as overhead_mb
                FROM information_schema.tables 
                WHERE table_schema = DATABASE()
            ";
            $overhead_result = $wpdb->get_var($overhead_query);
        }
        
        $overhead = ($overhead_result ?: 0) . ' MB';
        
        // Calculate fragmentation level
        $fragmentation_level = 0;
        if ($total_size_mb > 0) {
            $fragmentation_level = min(100, round(($overhead_result / $total_size_mb) * 100, 1));
        }
    ...
    /**
     * Get detailed revisions count
     */
    public function get_revisions_count_detailed($request) {
        global $wpdb;
        ...
        // Get revisions by post type
        $revisions_by_type = $wpdb->get_results("
        ...
        $revisions_by_post_type = array();
        foreach ($revisions_by_type as $type) {
            $revisions_by_post_type[] = array(
        ...
        return rest_ensure_response(array(
            'total_revisions' => intval($total_revisions),
            'posts_with_revisions' => intval($posts_with_revisions),
            'oldest_revision' => $oldest_revision,
            'revisions_by_post_type' => $revisions_by_post_type
        ));
    }
     ...
     */
    public function perform_optimization($request) {
        $params = $request->get_json_params();
        ...
        switch ($action) {
            ...
            case 'clean_revisions':
                ...
                return $this->clean_old_revisions($request);
                
            case 'clean_spam':
                return $this->clean_spam_comments($request);
                
            case 'clean_trash':
                return $this->clean_trash_posts();
            ...
            default:
                return new WP_Error('invalid_action', 'Invalid optimization action', array('status' => 400));
        }
    }
    ...
    private function optimize_database_tables() {
        ...
        if (!current_user_can('manage_options')) {
            return new WP_Error('insufficient_permissions', 'You do not have permission to optimize database', array('status' => 403));
        }
        ...
        update_option('wrm_last_optimized', current_time('mysql'));
        
        $this->log_security_event('database_optimized', "Optimized {$optimized_count} database tables");
        
        return rest_ensure_response(array(
            'success' => true,
            'message' => "Successfully optimized {$optimized_count} database table(s)"
        ));
    }
    ...
    private function clean_trash_posts() {
        ...
        if (!current_user_can('delete_posts')) {
            return new WP_Error('insufficient_permissions', 'You do not have permission to clean trash', array('status' => 403));
        }
        ...
        $this->log_security_event('trash_cleaned', "Cleaned {$deleted_count} trash items");
        
        return rest_ensure_response(array(
            'success' => true,
            'message' => "Successfully cleaned {$deleted_count} trash item(s)",
            'deleted_count' => $deleted_count
        ));
    }
    ...
    private function clean_expired_transients() {
        ...
        if (!current_user_can('manage_options')) {
            return new WP_Error('insufficient_permissions', 'You do not have permission to clean transients', array('status' => 403));
        }
        ...
        $this->log_security_event('transients_cleaned', "Cleaned {$deleted_count} expired transients");
        
        return rest_ensure_response(array(
            'success' => true,
            'message' => "Successfully cleaned {$deleted_count} expired transient(s)",
            'deleted_count' => $deleted_count
        ));
    }
    ...
    private function clean_orphaned_meta() {
        ...
        if (!current_user_can('manage_options')) {
            return new WP_Error('insufficient_permissions', 'You do not have permission to clean orphaned meta', array('status' => 403));
        }
        ...
        $this->log_security_event('orphaned_meta_cleaned', "Cleaned {$deleted_count} orphaned metadata entries");
        
        return rest_ensure_response(array(
            'success' => true,
            'message' => "Successfully cleaned {$deleted_count} orphaned metadata entrie(s)",
            'deleted_count' => $deleted_count
        ));
    }
    ...
    private function get_database_size() {
        ...
        return $size_mb ? $size_mb . ' MB' : 'Unknown';
    }
    
    /**
     * Log security event
     */
    private function log_security_event($event_type, $message, $object_id = null) {
        $log_entry = array(
        ...
        // Store in WordPress option (rotate logs to prevent bloat)
        $security_logs = get_option('aiowebcare_security_logs', array());
        ...
        update_option('aiowebcare_security_logs', $security_logs);
        
        // Also log to WordPress error log for critical events
        if (in_array($event_type, array('invalid_api_key', 'rate_limit_exceeded', 'insufficient_permissions', 'security_violation'))) {
            error_log(sprintf('AIOWebcare Security Event [%s]: %s (IP: %s)', $event_type, $message, $log_entry['ip_address']));
        }
    }
